import { useMutation, useFlash } from '@redwoodjs/web';
import { navigate, routes } from '@redwoodjs/router';
import OrderLineItemForm from 'src/components/OrderLineItemForm';

const CREATE_ORDER_LINE_ITEM_MUTATION = gql`
  mutation CreateOrderLineItemMutation($input: CreateOrderLineItemInput!) {
    createOrderLineItem(input: $input) {
      id
    }
  }
`;

const NewOrderLineItem = () => {
  const { addMessage } = useFlash();
  const [createOrderLineItem, { loading, error }] = useMutation(
    CREATE_ORDER_LINE_ITEM_MUTATION,
    {
      onCompleted: () => {
        navigate(routes.orderLineItems());
        addMessage('OrderLineItem created.', { classes: 'rw-flash-success' });
      },
    }
  );

  const onSave = (input) => {
    const castInput = Object.assign(input, {
      orderId: parseInt(input.orderId),
      productId: parseInt(input.productId),
    });
    createOrderLineItem({ variables: { input: castInput } });
  };

  return (
    <div className="rw-segment">
      <header className="rw-segment-header">
        <h2 className="rw-heading rw-heading-secondary">New OrderLineItem</h2>
      </header>
      <div className="rw-segment-main">
        <OrderLineItemForm onSave={onSave} loading={loading} error={error} />
      </div>
    </div>
  );
};

export default NewOrderLineItem;
